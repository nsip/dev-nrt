...
// of 205 char lines.
//
func NswWritingPearsonY7Report(cfh codeframe.Helper) *NswWritingPearsonY7 {

	r := NswWritingPearsonY7{cfh: cfh}
	r.initialise("./config/NswWritingPearsonY7.toml")
	r.printStatus()
...
func (r *NswWritingPearsonY7) ProcessEventRecords(in chan *records.EventOrientedRecord) chan *records.EventOrientedRecord {
	...
	go func() {
		...
		for eor := range in {
			...
			if eor.GetValueString("CalculatedFields.YrLevel") != "7" { // only for yr 5
				out <- eor
				continue
			}
...
//
//
func (r *NswWritingPearsonY7) calculateFields(eor *records.EventOrientedRecord) []byte {
