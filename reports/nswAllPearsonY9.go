...
func NswAllPearsonY9Report() *NswAllPearsonY9 {
	...
	r := NswAllPearsonY9{}
	r.initialise("./config/NswAllPearsonY9.toml")
	r.printStatus()
...
func (r *NswAllPearsonY9) ProcessStudentRecords(in chan *records.StudentOrientedRecord) chan *records.StudentOrientedRecord {
	...
	go func() {
		...
		for sor := range in {
			...
			if sor.GetValueString("CalculatedFields.YrLevel") != "9" { // only for yr 3
				out <- sor
				continue
			}
...
//
//
func (r *NswAllPearsonY9) calculateFields(sor *records.StudentOrientedRecord) []byte {
