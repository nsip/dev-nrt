...
func SystemTestAttemptsReport() *SystemTestAttempts {
	...
	r := SystemTestAttempts{}
	r.initialise("./config/SystemTestAttempts.toml")
	r.printStatus()
...
func (r *SystemTestAttempts) ProcessEventRecords(in chan *records.EventOrientedRecord) chan *records.EventOrientedRecord {
	...
	go func() {
		...
		for eor := range in {
			...
			// only sanctioned abandonment events
			if eor.GetValueString("NAPEventStudentLink.ParticipationCode") != "S" {
				out <- eor
				continue
			}
...
//
//
func (r *SystemTestAttempts) calculateFields(eor *records.EventOrientedRecord) []byte {
