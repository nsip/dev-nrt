...
func QaSchoolsReport() *QaSchools {
	...
	r.initialise("./config/QaSchools.toml")
	r.printStatus()
...
func (r *QaSchools) calculateFields(schoolAcaraId schoolId, schoolinfo []byte) []byte {
	...
	path = "CalculatedFields.QaSchools.TotalStudents"
	...
	for ssyl, count := range r.yrLevelCounter {
		...
		path = fmt.Sprintf("CalculatedFields.QaSchools.YearLevel.%s.TotalStudents", ssyl.year)
	...
	for sstl, count := range r.testLevelCounter {
		...
		path = fmt.Sprintf("CalculatedFields.QaSchools.TestLevel.%s.TotalStudents", sstl.level)
		json, _ = sjson.SetBytes(json, path, count)
	}

	// testAttemptsCounter
	attemptsCount := r.testAttemptsCounter[schoolAcaraId]
	path = "CalculatedFields.QaSchools.TotalTestAttempts"
	...
	for sstd, count := range r.domainAttemptsCounter {
		...
		path = fmt.Sprintf("CalculatedFields.QaSchools.TestLevel.%s.TestDomain.%s.TotalAttempts", sstd.level, sstd.domain)
		json, _ = sjson.SetBytes(json, path, count)
	}
	...
	for ssp, count := range r.participationCounter {
		...
		path = fmt.Sprintf("CalculatedFields.QaSchools.ParticipationCode.%s.TotalAttempts", ssp.pcode)
		json, _ = sjson.SetBytes(json, path, count)
	}

	// disruptionsCounter
	disruptionsCount := r.disruptionsCounter[schoolAcaraId]
	path = "CalculatedFields.QaSchools.TotalDisruptions"
	...
	for sstlx, count := range r.writingExtractCounter {
		...
		path = fmt.Sprintf("CalculatedFields.QaSchools.TestLevel.%s.TotalExtracts", sstlx.level)
		json, _ = sjson.SetBytes(json, path, count)
	}
...
//
func (r *QaSchools) collectSummary(eor *records.EventOrientedRecord) {
