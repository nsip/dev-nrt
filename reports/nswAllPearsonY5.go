...
func NswAllPearsonY5Report() *NswAllPearsonY5 {
	...
	r := NswAllPearsonY5{}
	r.initialise("./config/NswAllPearsonY5.toml")
	r.printStatus()
...
func (r *NswAllPearsonY5) ProcessStudentRecords(in chan *records.StudentOrientedRecord) chan *records.StudentOrientedRecord {
	...
	go func() {
		...
		for sor := range in {
			...
			if sor.GetValueString("CalculatedFields.YrLevel") != "5" { // only for yr 3
				out <- sor
				continue
			}
...
//
//
func (r *NswAllPearsonY5) calculateFields(sor *records.StudentOrientedRecord) []byte {
